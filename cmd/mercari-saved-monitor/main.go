...
const (
	defaultSavedStateFile = "state/mercari_saved_monitor.json"
...
func main() {
	...
	if err := mercariwatch.LoadDotEnv(mercariwatch.EnvString("ENV_FILE", filepath.Join("scripts", "mercari_saved_monitor.env"))); err != nil {
	...
	results, err := cari.ScanSavedQueries(ctx, cfg.OnlyNew, cfg.ScanLimit)
	if err != nil {
		log.Fatalf("scan saved queries: %v", err)
	}

	candidates := flattenSavedQueryResults(results)
	state, err := mercariwatch.LoadState(cfg.StateFile)
	if err != nil {
		log.Fatalf("load state: %v", err)
	}

	if len(state.Entries) == 0 && len(candidates) > 0 && cfg.BootstrapOnEmptyState {
		...
		log.Printf("bootstrapped %d saved-query listing(s); no alerts sent", len(candidates))
		return
	}
	...
	if len(pending) == 0 {
		log.Printf("no new or changed saved-query listings")
		return
	}
	...
	log.Printf("processed %d saved-query listing(s), sent %d finding alert(s), %d deal alert(s)", len(pending), generalSent, dealSent)
}

func loadConfig() config {
	return config{
		CariBin:               mercariwatch.EnvString("CARI_BIN", "cari"),
		StateFile:             mercariwatch.EnvString("STATE_FILE", defaultSavedStateFile),
		OnlyNew:               mercariwatch.EnvBool("ONLY_NEW", true),
		ScanLimit:             mercariwatch.EnvInt("SCAN_LIMIT", 10),
...
func flattenSavedQueryResults(results []mercariwatch.SavedQueryScanResult) map[string]candidate {
	out := make(map[string]candidate)
	for _, result := range results {
		label := strings.TrimSpace(result.SavedQueryName)
		if label == "" {
			label = fmt.Sprintf("Saved Query %d", result.SavedQueryID)
		}
